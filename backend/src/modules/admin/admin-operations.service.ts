...
export const searchAdmin = async (q: string, page: number, limit: number) => {
  ...
  const [visitors, officers, prisoners, appointments, paroles, changes, passes, firs, medical, visitorSupport, prisonerSupport, rules] = await prisma.$transaction([
    prisma.visitorProfile.findMany({ where: { OR: [{ publicId: contains }, { name: contains }, { user: { email: contains } }] }, take: limit, select: { publicId: true, name: true, user: { select: { email: true } } } }),
    prisma.officerProfile.findMany({ where: { OR: [{ publicId: contains }, { name: contains }, { user: { email: contains } }] }, take: limit, select: { publicId: true, name: true, user: { select: { email: true } } } }),
    prisma.prisonerProfile.findMany({ where: { OR: [{ publicId: contains }, { name: contains }, { user: { email: contains } }] }, take: limit, select: { publicId: true, name: true, user: { select: { email: true } } } }),
  ...
  const profileResults = (type: string, base: string, items: Array<{ publicId: string | null; name: string; user: { email: string | null } }>) => items.filter((i) => i.publicId).map((i) => ({ type, reference: i.publicId!, title: i.name, subtitle: i.user.email, href: `${base}/${i.publicId}` }));
  const results = [
  ...
  return { groups: Object.values(results.reduce<Record<string, { type: string; items: typeof results }>>((all, item) => { (all[item.type] ??= { type: item.type, items: [] }).items.push(item); return all; }, {})), pagination: pageInfo(page, limit, results.length) };
};
