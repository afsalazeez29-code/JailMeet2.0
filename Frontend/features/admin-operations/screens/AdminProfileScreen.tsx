...
export default function AdminProfileScreen(){
  ...
  return <div className={styles.page}><div className={styles.header}><div><h1>My Profile</h1><p className={styles.muted}>Read-only permanent Admin account information</p></div></div><section className={styles.profileCard}><div className={styles.profileImageColumn}><img alt={`${data.name} profile`} src={data.profileImageUrl?.trim()||'/images/avatars/admin-default.PNG'} onError={(event)=>{if(event.currentTarget.dataset.fallbackApplied==='true')return;event.currentTarget.dataset.fallbackApplied='true';event.currentTarget.src='/images/avatars/admin-default.PNG';}}/><strong>ADMIN</strong></div><dl className={styles.profileGrid}>{fields.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></section><section className={`${styles.card} mt-3`}><h2>Recent Activity</h2>{data.recentActivity.length?<div className={styles.activityList}>{data.recentActivity.map((item)=><article key={`${item.createdAt}:${item.action}`}><strong>{item.action} · {item.entity}</strong><span>{item.entityReference||'No public reference'} · {item.result||'No result'}</span><time dateTime={item.createdAt}>{date(item.createdAt)}</time></article>)}</div>:<p>No recent activity.</p>}</section></div>;
}
