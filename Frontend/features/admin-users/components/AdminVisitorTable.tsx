        ...
        <tbody>
          {visitors.map((visitor) => (
            <tr key={visitor.publicId ?? visitor.user.email ?? visitor.name}>
              <td>{formatVisitorPublicId(visitor.publicId)}</td><td><div className={styles.identity}><img alt="" className={styles.avatar} src={visitor.profilePic || '/images/avatars/visitor-default.png'}/><span>{visitor.name}</span></div></td><td>{visitor.user.email}</td><td>{visitor.phone}</td><td>{visitor.state || 'N/A'}</td>
