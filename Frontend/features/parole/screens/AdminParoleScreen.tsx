...
import { getAdminParoleRequests } from '@features/parole/services/parole.service';
import { isApiServiceError } from '@/types/api';
import { AdminParoleRequest, PaginatedResponse } from '@features/parole/types';
import { ParoleStatus } from '@features/parole/types';
import AdminFilters from '../../../components/common/AdminFilters';
import AdminParoleTable from '@features/parole/components/AdminParoleTable';
...
export default function AdminParolePage() {
  ...
  if (protectedPage.isLoading || loading || (!protectedPage.isReady && !protectedPage.error && !protectedPage.isForbidden)) return <Shell><LoadingAlert>Loading parole requests...</LoadingAlert></Shell>;
  if (protectedPage.isForbidden || error === 'Access denied') return <Shell><ForbiddenAlert /></Shell>;
  return <Shell><h3 className="fw-bold mb-3">Parole Requests</h3>{error || protectedPage.error ? <ErrorAlert>{error || protectedPage.error}</ErrorAlert> : null}<AdminFilters search="" onSearchChange={() => undefined} selects={[{ label: 'Status', value: status, options: statusOptions, onChange: (value) => { setStatus(value); setPage(1); } }]} /><div className="card"><div className="card-body"><AdminParoleTable requests={data?.items ?? []} />{data ? <Pagination pagination={data.pagination} onPageChange={setPage} /> : null}</div></div></Shell>;
}
